...
import { Alert, Button, CircularProgress, LinearProgress, Modal, Typography } from '@mui/material'
...
const Calendar: React.FC = () => {
	...
	function contentChange(e: { target: { value: React.SetStateAction<string> } }) {
				...
				{posts.map(
					(post: {
						...
						if (eventDay === post.date) {
							...
							return (
								<li key={post.length}>
									<EventLists title={post.title} content={post.content} date={post.date} id={post.length} />
								</li>
							)
						}
					}
			...
			>
				<Box style={{ backgroundColor: 'white' }}>
					...
					<form onSubmit={(e) => submitHandler(e)}>
						<input type="text" value={eventTitle} onChange={(e) => titleChange(e)} />
