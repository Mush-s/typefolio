...
const FriendMemo = (props: Props) => {
  ...
  function changeHandler(e: React.ChangeEvent<HTMLInputElement>, id: string) {
    console.log(id);
    // if (postData[id].done === false) {
    //   UpdatePost({
    //     variables: {
    //       lengthId: id,
    //       done: true,
        ...
        {filteredAllPosts.map((post) => (
          <li key={post.length} className={styles.block}>
            <h3 className={styles.title}>
              {post.title}
              <Checkbox onChange={(e) => changeHandler(e, post.title)} />
            </h3>
