...
const FriendMemo = (props: Props) => {
  ...
  return (
    <div className={styles.friendmemo}>
        <div className={styles.name}>Name: {name}</div>
      <ul >
        {filteredPosts.map((post) => (
          <li key={post.length} className={styles.block}>
            <div className={styles.content}>Date: {post.date}</div>
            <div className={styles.content}>Title: {post.title}</div>
            <div className={styles.content}>Content: {post.content}</div>
          </li>
        ))}
      </ul>
    </div>
